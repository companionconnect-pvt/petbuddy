...
const PethouseProfile = () => {
  ...
  const fetchPethouseProfile = async () => {
    setIsLoading(true);
    try {
      ...
      console.log("Fetched pethouse profile:", res.data);
  ...
  const handleSaveClick = async () => {
    ...
    try {
      ...
      setPethouse({ ...pethouse, [editingField]: editValue });
      ...
      {editingField === fieldName ? (
        <div className="flex items-center space-x-3">
          <input
            ...
            className="flex-1 border border-gray-200 px-4 py-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-pink-500"
          />
          <button
            onClick={handleSaveClick}
            className="px-4 py-3 text-pink-600 border border-pink-600 rounded-lg hover:bg-pink-600 hover:text-white transition-all duration-300"
          >
            Save
          </button>
        </div>
    ...
    return (
      <div className="flex justify-center items-center min-h-screen bg-gray-50">
        <motion.div
          ...
          className="rounded-full h-16 w-16 border-t-4 border-b-4 border-pink-500"
        ></motion.div>
      </div>
    );
    ...
    return (
      <div className="text-center mt-10 text-gray-500">
        ...
        <button
          ...
          className="mt-4 px-6 py-2 text-pink-600 border border-pink-600 rounded-lg hover:bg-pink-600 hover:text-white transition-colors duration-200"
        >
          Retry
        </button>
      </div>
    ...
    <div className="p-6 md:p-10 bg-gray-50 min-h-screen">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        >
          ...
          <p className="text-gray-500 mb-4">{pethouse.email}</p>
        ...
        <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
          <AnimatePresence>
            ...
            {renderEditableField(
              "Address",
              "address",
              pethouse.address.street +
                ", " +
                pethouse.address.city +
                ", " +
                pethouse.address.state +
                ", " +
                pethouse.address.zip,
              <FiMapPin size={20} />,
              "address"
            )}
            {/* Add more editable fields as per your schema */}
          </AnimatePresence>
        </div>
      </div>
    </div>
